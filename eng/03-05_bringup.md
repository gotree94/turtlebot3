...
4. Topics and services can be listed with the commands below. 
  * TheRemote PCcan subscribe topics published by theTurtlebot3 SBCby connecting both to the same network environment. 
...
### 3.5.2 Bringup TurtleBot3

1. Open a new terminal on the remote PC with `Ctrl` + `Alt` + `T` and connect to Raspberry Pi using SSH.  The default password is **turtlebot** . 
**[Remote PC]** 
```
$ ssh ubuntu@{IP_ADDRESS_OF_RASPBERRY_PI}  
```
...
3. If you used git clone for to install the TurtleBot3 packages, enter the command below. You will need to specify the model of your TurtleBot3
**[Remote PC]**
```
